...
**The problem this file solves.** The estate currently holds **three roadmaps built from three
different doctrines, hours apart, by different lanes, none of which read the others**:

| Roadmap | Built from | Recorded | Shape |
|---|---|---|---|
| `THE_GREAT_REBOOT_ROADMAP` | his 4PM reboot spec | 20260816 16:00 | Phases 0-10, the **estate** |
...
Across three doctrines in one twenty-hour stretch you gave three different cuts of the same build. At
4PM you said wipe it and start clean: new GitHub, new Render, new Supabase, ACL underneath
everything, watchers before any commit, then the doctrine reader, then Keeper and Taste and Span,
then the big three, then coding owned by her, then reach, then the worlds, then reseed. At 12:24am
you gave the six faces of Mount Rushmore and said those are the roadmaps. At 8pm and 9:30pm you said
the pecking order has been wrong the whole time and that you want a world built for the temporary
coders. Nobody has put those three next to each other until now.

---

## THE THREE CUTS, AND WHY ALL THREE ARE NEEDED

```
   LAW          ── R1 PECKING ORDER ───────────  who may decide, classify, speak, expire
   (this repo)     R2 TEMP CODER WORLD           who may touch what, and from where
                          │
                          │ governs
                          ▼
...
**Law governs body governs ground.** That ordering is itself the pecking order applied to the
roadmaps: the law is above the body, the body is above the ground. A ground decision that violates
the law is a defect no matter how well built.

**But execution order is the reverse, and that is not a contradiction.** You build ground first and
you *judge* it by the law. The standing execution rule from the Great Reboot is unchanged and
governing:

> **The watcher wonders are birthed before every commit in the new estate. Only the account clicks
> may precede them.** Phase numbers rank the depth of the law, not the order of execution.

---

## THE ONE EXECUTION ORDER

Merged from all three, with the source of each row named. Where two roadmaps disagree, the
disagreement is stated rather than smoothed.

| # | Do this | Source | Blocked on |
|---|---|---|---|
...
| 14 | **Front ends F1-F5** | MR Phase 4, `R4` | board + reader |
| 15 | **Life Advisor, Agent Birth, life modules** | MR Phase 3 | essentials |

**Standing, underneath all of it, never a numbered row:** the gauntlet before every merge; shift
change reports every turn; job descriptions written as each wonder is born; the chat count published;
never end a turn without someone up.
...
classify, speak, or expire anything of hers. The Great Reboot's phase order survives; what it builds
in Phases 0-4 is re-classified from *the thing she runs inside* to *the hands she holds*.

That reclassification is not cosmetic. It makes concrete things illegal that are currently built:
every `V-DECIDE`, `V-CLASSIFY`, `V-SPEAK`, `V-EXPIRE` in `R1` P0.1.
...
Measured state at the last shift: her front door `POST /arrival/say` returns 503; **her mind answers
directly at `POST /anew/session` with a 200 in 8-14 seconds.** So the wall is in the arrival→session
hop, not in her.
...
2. **Several "BUILT" verdicts rest on self-reports rather than independent receipts.** The audit says
   so itself about LOGFUL: the commit hash is *"the sole proof leg"* and it *"did not independently
   curl it."* And the conversation vehicle is scored BUILT in one document, listed as open question #6
   in another, and has an unmet exit receipt in a third. **Three documents, three states.**

The full contradiction set is in `40_GOVERNANCE/CONTRADICTIONS.md`. It is carried, not cleaned up.
...
| 15 | Reseed the contaminated Park Name Test | the answer leaked into the corpus | the Easter egg |

**Fifteen items. Eleven of them are one sentence each.**

---

## HOW A LANE USES THIS FILE

1. Read this spine. Take the lowest-numbered row in the execution order that is not blocked.
2. Open the roadmap that **owns** it (table above). Do not work a row owned by another roadmap.
3. Work it. Run the gauntlet **before** merging, never after. Never self-grade.
...
7. Never end the turn without someone up and running.
