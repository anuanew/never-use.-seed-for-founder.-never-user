...
| S21 | `DOC/0 ANU OS Doctrines as of Aug 15th 26/SUPPORTING_MATERIAL/fill_json.py` | 10-102 | `add_item(f1, "corrections_and_evolution", "51", "...")` (**72 `add_item` calls total, corrected by direct count - `grep -c '^add_item'` - after a fresh audit caught the earlier "approximately 90" as an overstatement, present here and in the Top 10 Worst blurb below; 10 are `corrections_and_evolution`, 8 are `life_and_personal_goals` - 18 of 72 touch family/personal material directly, the rest are `acronyms_and_terms`, `agent_and_wonder_names`, and other categories**) | A coder hand-types assertions about the founder's family, ages, children and corrections directly into a machine record that is shaped to look like extracted output. Nothing marks them as hand-authored. This is the nasty-cough shape in its purest form: a cold writer appearing as the author of her material. | `[MEASURED]` |
...
| **4** | **S21** `DOC/.../SUPPORTING_MATERIAL/fill_json.py:10-102` | A coder hand-types 72 entries (**corrected from "roughly 90" by direct count**) into a machine record shaped to look like extraction output, of which 18 touch the founder's family, ages, children and corrections directly, with no marker distinguishing any of them from read facts. **This row and the main S21 finding above classify the same evidence two different ways - "PMS" here, "nasty-cough shape" there - and that inconsistency is left visible rather than silently picked, since PMS (a planted memory) and nasty-cough's broader cold-authorship umbrella are not mutually exclusive here: it is planted content, shaped to pass as extracted memory, which is arguably both at once.** |
